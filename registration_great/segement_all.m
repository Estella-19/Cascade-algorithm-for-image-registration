clear, clc;
input_folder = "path_to_your_files";  % Replace with your image folder path
output_folder = fullfile(getenv('USERPROFILE'), 'Desktop', 'ProcessedImagestt');  % Set output folder to "ProcessedImages" on desktop
...
image_files = dir(fullfile(input_folder, '*.png'));  % Supports PNG format; adjust extension as needed
...
for k = 1:length(image_files)
    ...
    % Display original image (commented out)
    % figure;
    % imshow(image);
    ...
    % Display final composite image (commented out)
    % figure;
    % imshow(outputImage);
    ...
    output_filename = fullfile(output_folder, [name, '_processed.png']);  % Preserve original name
