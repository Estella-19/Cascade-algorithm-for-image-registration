...
gray_output_folder = "path_to_your_files";  % Folder path to save grayscale images
...
for k = 1:length(image_files)
    ...
    for channel = 1:3
        filtered_png1(:,:,channel) = medfilt2(png1(:,:,channel), [5 5]); % 5x5 median filter
    end
    ...
    sub_1 = uint8(zeros(size(filtered_png1))); % Initialize all-zero image
    sub_1(repmat(combined_mask_1, [1, 1, 3])) = filtered_png1(repmat(combined_mask_1, [1, 1, 3]));

    % Calculate normalized cross-correlation (NCC)
    red_channel_png2 = png2(:,:,1);  % Extract red channel from png2
    ...
    template_area = sum(template(:) > 0); % Template area (non-zero pixels)
    overlap_area = sum(resized_template(:) > 0 & overlap_region(:) > 0); 

    overlap_ratio = overlap_area / template_area;  
    overlap_percentage = min(overlap_ratio * 100, 100);  % Cap at 100%

    % Reduce red channel intensity in png2
    png2_reduced = png2;
    png2_reduced(:,:,1) = png2_reduced(:,:,1) * 0.5;  % Dim red channel
    ...
    for i = 1:size(resized_template, 1)
        for j = 1:size(resized_template, 2)
            if i <= size(overlay_image, 1) && j <= size(overlay_image, 2)
                overlay_image(ybegin + i - 1, xbegin + j - 1, 1) = resized_template(i, j, 1);  % Red channel overlay
                overlay_image(ybegin + i - 1, xbegin + j - 1, 2:3) = max(overlay_image(ybegin + i - 1, xbegin + j - 1, 2:3), resized_template(i, j, 2:3));  % Preserve max values
            end
        end
    end
