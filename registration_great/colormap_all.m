clear, clc;
% Set folder paths
folder_path = "path_to_your_files";  % Replace with your image folder path
output_folder = fullfile(getenv('USERPROFILE'), 'Desktop', 'ColormapImageszcf'); 

% Get all image files in the folder
image_files = dir(fullfile(folder_path, '*.png'));  % Supports PNG format; can change extension as needed
...
for k = 1:length(image_files)
    ...
    idx = cellfun(@(x) x(1), idx, 'UniformOutput', true);  % Take first index per result

    % Reconstruct RGB image
    RGB_map = cmap(idx, :); % Map to RGB colors
