% Define folder path (replace with your image folder path)
folderPath = "path_to_your_files"; 

% Get all image files in folder (supports common formats)
imageFiles = [dir(fullfile(folderPath, '*.jpg')); 
             dir(fullfile(folderPath, '*.png'));
             dir(fullfile(folderPath, '*.tif'))];

% Natural sorting function (handles filenames with numbers)
...
% Initialize result storage (contains all images)
ssimResults = table('Size', [length(imageFiles), 2], ...
    'VariableTypes', {'string', 'double'}, ...
    'VariableNames', {'ImageName', 'SSIM'});

% Calculate SSIM for all images (including reference image itself)
for i = 1:length(imageFiles)
    try
        ...
        % Unify image dimensions (match reference image)
        if ~isequal(size(currentImage), refSize)
            currentImage = imresize(currentImage, refSize);
        end
        
        % Calculate SSIM (handles data types automatically)
        ...
        ssimResults.ImageName(i) = imageFiles(i).name;
        ssimResults.SSIM(i) = NaN;  % Mark errors with NaN
    end
end
