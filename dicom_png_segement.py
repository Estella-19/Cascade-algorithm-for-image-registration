    ...
    for root, _, files in os.walk(folder_path):
        for file in files:
            ...
            try:
                ...
                ds.file_path = file_path  # Save the original path
                if 'PixelData' in ds:
                    dicom_files.append(ds)
            except:
                continue

    # Sort by InstanceNumber (use file name if not available)
    try:
...
def select_roi(image_array):
    ...
    scale_factor = 10  # 500% magnification factor

    # Convert the original image to a PIL image and enlarge it
    img_pil = Image.fromarray(image_array)
    enlarged_pil = img_pil.resize(
        (img_pil.width * scale_factor, img_pil.height * scale_factor),
        Image.Resampling.LANCZOS  # High-quality interpolation
    )
    enlarged_array = np.array(enlarged_pil)  # Convert back to a numpy array

    root = tk.Tk()
    root.title("Draw ROI on Enlarged Image (500%) - Press Enter to Confirm")

    fig, ax = plt.subplots(figsize=(10, 10))  # Increase the canvas size appropriately
    ...
    if roi_coords['start'] and roi_coords['end']:
        # Sort the coordinates (ensure x1 < x2, y1 < y2)
